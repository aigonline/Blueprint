...
const DesignLayoutSchema = z.object({
  ...
  elements: z.array(
    z.object({
      ...
      size: z
        .object({
          width: z.number().min(0.1, "Width must be at least 0.1.").describe('The width of the element, must be at least 0.1.'),
          height: z.number().min(0.1, "Height must be at least 0.1.").describe('The height of the element, must be at least 0.1.'),
        })
        .describe('The size of the element. Both width and height must be positive values (>=0.1).'),
...
All elements must have a width and height of at least 0.1.
